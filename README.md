...
The `unlift` method is only a marker that indicates that the `lift` macro transformation needs to treat a value as monad instance. For example, it never blocks thread using `Await.result` if it's dealing with a `Future`. 
